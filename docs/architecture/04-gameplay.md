...
flowchart TD
    ...
    PLAYER -->|Разыграть карту| PLAYCARD[combatReducer: PlayCard<br/>оплата энергии → applyCard → перемещение карты]
...
> Реализация этого цикла — `combatReducer` (поверх `applyCard` + `DeckManager` + op-ов сущностей; см. [архитектуру домена §5](02-architecture.md#5-архитектура-домена)). Пошаговая последовательность взаимодействий — в [§12.1](05-generation-combat.md#121-структура-хода).
