...
> Детерминизм и тестируемость напрямую опираются на чистоту доменных сервисов (см. [§5](02-architecture.md#5-архитектура-классов)) и сид-генерацию (см. [§11](05-generation-combat.md#11-процедурная-генерация-уровня)).
...
| Раздувание `GameStore` | Срезы (slices) + изоляция чистой логики в `domain/` (см. [§6.2](02-architecture.md#62-структура-стора-zustand-срезы)). |
